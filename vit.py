...
#initializing radiomic features global dictionary
...
#storing validation data radiomic feature in dictionary
...
#storing training data radiomic features in dictionary
...
#getting ready to extract features
...
#applying watershed segmentation on training data to get the mask of an image
...
#applying watershed segmentation on validation data to get the mask of an image
...
#not applying data augmentation on validation data
...
#not applying data augmentation on training data
def train_no_data_augmentation(normal_files,covid_files,pneumonia_files,tb_files,cat_files):
    aug_normal=[]
    aug_covid=[]
    aug_pneumonia=[]
    aug_tb=[]
...
#if for future work have to try normal augmentation with radiomic features
def my_augmenter(x):
    my_list=[]
...
#function for applying normal augmentation then applying advance augmentation for future work
...
#applying cutmix data augmentation technique for future work 
...
import cv2
IM_SIZE=256
#applying mixup advance data augmentation for future work
...
#helping future authors with cutout advance data augmentation for future work
...
#applying advance augmentation with radiomic feature to help future authors   
...
#plotting curves for accuracy and loss curves 
...
#combining validation data into a list
...
#combining training data into a list
...
#inception_pred.compile(optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.0002), loss=tf.keras.losses.CategoricalCrossentropy(from_logits = False) , metrics=['accuracy'])
    ...
    my_fold=0
    model_counter=0
    #doing 80:20 training and testing split
    ...
    #train_radiomic_features=train_my_dict
    #val_radiomic_features=val_my_dict
    ...
    reshaped_features = Reshape((-1, vit_features.shape[-1]))(vit_features)
    ...
    history= Model.fit(train_full_data,  # Replace with your training data
    train_full_label,  # Replace with your training labels
    batch_size=32,  # Adjust as needed
    epochs=30,# Adjust as needed
    validation_data=(val_full_data, val_full_label),  # Replace with your validation data
    callbacks=[tqdm_callback, early_stopping])

    #getting the results
    pred=Model.predict([val_full_data])
    ...
    numpy_path='/home/ssharma8/manas_user/history_vit'+str(model_counter)+'.npy'
    ...
    df.to_csv("/home/ssharma8/manas_user/vit_results.csv")
    model_counter+=1   
        
    zipped = list(zip(accuracy_d, precision_d, recall_d, f_score_d))

    df = pd.DataFrame(zipped, columns=['Accuracy', 'Precision', 'Recall', 'F_SCORE'])
    df.to_csv("/home/ssharma8/manas_user/vit_final_results.csv")
    filename = '/home/ssharma8/manas_user/vit_model_'+str(model_counter)+'.sav'
